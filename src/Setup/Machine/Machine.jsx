...
const Machine = () => {
  ...
  const handleExportCSV = () => {
    const csvData = machines.map(
      ({ id, companyName, plantName, machineName, creditedBy, isActive }) => ({
        ID: id,
  ...
  const handleBulkDelete = () => {
    const updatedMachines = machines.filter(
      (machine) => !selectedRows.includes(machine.id)
    );
  ...
  const handleBulkEdit = () => {
    alert(`Editing machines with IDs: ${selectedRows.join(", ")}`);
  };
  ...
  // Filter machines based on search, creditedBy, and isActive
  const filteredMachines = machines.filter((machine) => {
    const matchesSearch = machine.machineName
      .toLowerCase()
      .includes(search.toLowerCase());
    const matchesCreditedBy = creditedByFilter
      ? machine.creditedBy === creditedByFilter
      : true;
    const matchesIsActive = isActiveFilter
      ? machine.isActive.toString() === isActiveFilter
      : true;
        ...
        <Table>
          <TableHead>
            <TableRow sx={{backgroundColor:"rgba(143, 143, 143, 0.1)" }}>
              <TableCell padding="checkbox">
                <Checkbox
                  checked={selectedRows.length === filteredMachines.length}
                  onChange={() => {
                    if (selectedRows.length === filteredMachines.length) {
                      setSelectedRows([]);
                    } else {
                      setSelectedRows(filteredMachines.map((machine) => machine.id));
                    }
                  }}
                />
              </TableCell>
              ...
              {columnVisibility["Machine Name"] && <TableCell sx={{ width: "150px" }}>Machine Name</TableCell>}
          ...
          <TableBody>
            {paginatedMachines.map((machine) => (
              <TableRow key={machine.id} sx={{ '&:hover': { backgroundColor: '#e3f2fd', transition: 'background-color 0.3s ease' } }}>
                <TableCell padding="checkbox">
                  <Checkbox
                    checked={selectedRows.includes(machine.id)}
                    onChange={() => handleRowSelect(machine.id)}
                  />
                </TableCell>
                {columnVisibility["ID"] && <TableCell>{machine.id}</TableCell>}
                {columnVisibility["Company Name"] && <TableCell>{machine.companyName}</TableCell>}
                {columnVisibility["Plant Name"] && <TableCell>{machine.plantName}</TableCell>}
                {columnVisibility["Machine Name"] && <TableCell>{machine.machineName}</TableCell>}
                {columnVisibility["Credited By"] && <TableCell>{machine.creditedBy}</TableCell>}
                ...
                {columnVisibility["Actions"] && (
                  <TableCell>
                    <IconButton onClick={(e) => handleMenuOpen(e, machine)}>
                      <MoreVert />
                    </IconButton>
                    <Menu
                      anchorEl={menuAnchor}
                      open={Boolean(menuAnchor && selectedMachine === machine)}
                      onClose={handleMenuClose}
                    >
                      <MenuItem onClick={() => alert(`Editing ${machine.machineName}`)}>
                        <Edit sx={{ color: "green", mr: 1 }} /> Edit
                      </MenuItem>
                      <MenuItem onClick={() => alert(`Deleting ${machine.machineName}`)}>
